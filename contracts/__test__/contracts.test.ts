...
import { StakedInfo, StakingPoolClient, StakingPoolFactory, ValidatorPoolKey } from '../contracts/clients_new/StakingPoolClient'
import { MbrAmounts, ValidatorConfig, ValidatorRegistryClient, ValidatorRegistryFactory } from '../contracts/clients_new/ValidatorRegistryClient'
import {
  addStake,
  addStakingPool,
  addValidator,
...
describe('reti', () => {
  ...
  beforeAll(async () => {
    ...
    const { result, appClient } = await validatorFactory.send.create.createApplication({ args: [], extraProgramPages: 3 })
    ...
    const composer = validatorMasterClient.newGroup().initStakingContract({ args: { approvalProgramSize: stakingApprovalProgram.length } })
  ...
  describe('MultValidatorAddCheck', () => {
    ...
    test('validatorAddTests', async () => {
      ...
      let expectedID = 1
      let validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, mbrs.addValidatorMbr)
      ...
      expectedID += 1
      validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, mbrs.addValidatorMbr)
      expect(validatorId).toEqual(expectedID)
      expectedID += 1
      validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, mbrs.addValidatorMbr)
      expect(validatorId).toEqual(expectedID)
  ...
  describe('StakeAdds', () => {
    ...
    test('firstStaker', async () => {
      ...
      const origStakePoolInfo = await fixture.context.algorand.account.getInformation(getApplicationAddress(poolAppId))
      ...
      const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
      const [stakedPoolKey, fees1] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
      ...
      expect(poolBalance1.balance.microAlgo).toEqual(origStakePoolInfo.balance.microAlgo + stakeAmount1.microAlgo - mbrs.addStakerMbr)
      ...
      const [stakedKey2, fees2] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount2, 0n)
      ...
      poolInfo = await getPoolInfo(validatorMasterClient, firstPoolKey)
      expect(poolInfo.totalAlgoStaked).toEqual(stakeAmount1.microAlgos - mbrs.addStakerMbr + stakeAmount2.microAlgos)
      // and global state changed
      validatorGlobalState = await validatorMasterClient.state.global.getAll()
      expect(validatorGlobalState.totalAlgoStaked).toEqual(stakeAmount1.microAlgos - mbrs.addStakerMbr + stakeAmount2.microAlgos)
      ...
      const stateData = await getValidatorState(validatorMasterClient, validatorId)
      expect(stateData.numPools).toEqual(1n)
      expect(stateData.totalAlgoStaked).toEqual(stakeAmount1.microAlgos + stakeAmount2.microAlgos - mbrs.addStakerMbr)
      expect(stateData.totalStakers).toEqual(1n)
      // and. globally
      validatorGlobalState = await validatorMasterClient.state.global.getAll()
      expect(validatorGlobalState.totalAlgoStaked).toEqual(stakeAmount1.microAlgos + stakeAmount2.microAlgos - mbrs.addStakerMbr)
    })
    ...
    test('nextStaker', async () => {
      ...
      const [stakedPoolKey, fees] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
      // should be same as what we added prior
      expect(stakedPoolKey.id).toEqual(firstPoolKey.id)
      expect(stakedPoolKey.poolId).toEqual(firstPoolKey.poolId)
      expect(stakedPoolKey.poolAppId).toEqual(firstPoolKey.poolAppId)
      ...
      const poolBalance1 = await fixture.context.algorand.account.getInformation(getApplicationAddress(poolAppId))
      expect(poolBalance1.balance.microAlgo).toEqual(origStakePoolInfo.balance.microAlgo + stakeAmount1.microAlgos - mbrs.addStakerMbr)
      ...
      const stateData = await getValidatorState(validatorMasterClient, validatorId)
      expect(stateData.numPools).toEqual(1n)
      expect(stateData.totalAlgoStaked).toEqual(origValidatorState.totalAlgoStaked + stakeAmount1.microAlgos - mbrs.addStakerMbr)
    ...
    test('addMaxPoolsAndFill', async () => {
      ...
      for (let i = 0; i < poolsToCreate - 1; i += 1) {
        const [stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakers[i], stakeAmount, 0n)
      ...
      const stateData = await getValidatorState(validatorMasterClient, validatorId)
      consoleLogger.info(`num pools: ${stateData.numPools}, total staked:${stateData.totalAlgoStaked}, stakers:${stateData.totalStakers}`)
    ...
    test('addThenRemoveStake', async () => {
      ...
      expect(allPools[0]).toEqual(firstPoolKey)
      // ....and verify data for the 'staker' is correct as well
      const ourPoolClient = stakingPoolFactory.getAppClientById({ appId: firstPoolKey.poolAppId, defaultSender: stakerAccount.addr })
      ...
      const removeFees = await removeStake(ourPoolClient, stakerAccount, AlgoAmount.MicroAlgos(stakerInfo.balance))
  ...
  describe('StakeAddWMixedRemove', () => {
    ...
    test('addRemoveByStaker', async () => {
      ...
      let amountStaked = 0n
      const [addStake1] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, AlgoAmount.Algos(1100), 0n)
    ...
    for (let i = 0; i < stakersPriorToReward.length; i += 1) {
      ...
      if (timePercentage < 1000n) {
        ...
        if (origBalance + expectedReward !== stakersAfterReward[i].balance) {
          consoleLogger.warn(`staker:${i} expected: ${origBalance + expectedReward} reward but got: ${stakersAfterReward[i].balance}`)
  ...
  describe('StakeWRewards', () => {
    ...
    beforeAll(async () => {
      ...
      firstPoolClient = stakingPoolFactory.getAppClientById({ appId: firstPoolKey.poolAppId, defaultSender: validatorOwnerAccount.addr })
    ...
    test('firstStaker', async () => {
      ...
      // we pay the extra here so the final staked amount should be exactly 1000
      const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
      const [stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
      // should match info from first staking pool
      expect(stakedPoolKey.id).toEqual(firstPoolKey.id)
      expect(stakedPoolKey.poolId).toEqual(firstPoolKey.poolId)
      expect(stakedPoolKey.poolAppId).toEqual(firstPoolKey.poolAppId)
    ...
    test('testFirstRewards', async () => {
      ...
      expect(newOwnerBalance.balance.microAlgo).toEqual(ownerBalance.balance.microAlgo - fees.microAlgos + expectedValidatorReward)
      ...
      expect(newValidatorState.totalAlgoStaked).toEqual(origValidatorState.totalAlgoStaked + reward.microAlgos - expectedValidatorReward)
    ...
    test('extractRewards', async () => {
      ...
      expect(newStakerBalance.balance.microAlgo).toEqual(origStakerBalance.balance.microAlgo + AlgoAmount.Algos(1190).microAlgos - fees)
    ...
    test('testTooEarlyEpoch', async () => {
      ...
      const poolBalance = await getPoolAvailBalance(fixture.context, firstPoolKey)
      expect(poolBalance).toEqual(AlgoAmount.Algos(200).microAlgos - (AlgoAmount.Algos(100).microAlgos * BigInt(PctToValidator)) / 100n)
    ...
    test('testPartialReward', async () => {
      ...
      const [aPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccounts[0], stakeAmount1, 0n)
      ...
      expect(poolInfo.totalAlgoStaked).toEqual(stakeAmount1.microAlgos + partialStakersAmount.microAlgos * 2n - 2n * mbrs.addStakerMbr)
  ...
  describe('StakeW0Commission', () => {
    ...
    test('extractRewards', async () => {
      ...
      const newStakerBalance = await fixture.context.algorand.account.getInformation(stakerAccounts[0].addr)
      // 1000 algos staked + 190 reward (- fees for removing stake)
      expect(newStakerBalance.balance.microAlgo).toEqual(origStakerBalance.balance.microAlgo + expectedBalance.microAlgos - fees)
  ...
  describe('StakeWTokenWRewards', () => {
    ...
    beforeAll(async () => {
      ...
      rewardTokenId = await createAsset(fixture.context, tokenCreatorAccount, 'Reward Token', 'RWDTOKEN', 100_000, decimals)
      ...
      validatorId = await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, validatorConfig, mbrs.addValidatorMbr)
    ...
    test('extractRewards', async () => {
      ...
      const assetInfo = await fixture.context.algorand.asset.getAccountInformation(stakerAccounts[0].addr, rewardTokenId)
    ...
    test('testPartialReward', async () => {
      ...
      const stakeAmount2 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)

      // Add stake for partial-epoch staker
      const [newPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, partialEpochStaker, stakeAmount2, 0n)
      ...
      expect(poolInfo.totalAlgoStaked).toEqual(stakeAmount1.microAlgos + stakeAmount2.microAlgos - mbrs.addStakerMbr)
    ...
    test('reclaimTokenRewards', async () => {
      ...
      const rewardTokenBalance = await fixture.context.algorand.asset.getAccountInformation(pool1Address, rewardTokenId)
      ...
      const ownerTokenBalPre = await fixture.context.algorand.asset.getAccountInformation(tokenCreatorAccount.addr, rewardTokenId)
      ...
      const ownerTokenBal = await fixture.context.algorand.asset.getAccountInformation(tokenCreatorAccount.addr, rewardTokenId)
  ...
  describe('StakeUnstakeAccumTests', () => {
    ...
    test('stakeAccumTests', async () => {
      ...
      const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(2000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
      const [stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
  ...
  describe('TokenRewardOnlyTokens', () => {
    ...
    beforeAll(async () => {
      ...
      rewardTokenId = await createAsset(fixture.context, tokenCreatorAccount, 'Reward Token', 'RWDTOKEN', 100_000, 0)
    ...
    test('extractRewards', async () => {
      ...
      // verify that reward token payout came to us
      const assetInfo = await fixture.context.algorand.asset.getAccountInformation(stakerAccount.addr, rewardTokenId)
      expect(assetInfo.balance).toEqual(tokenRewardPerPayout)
  ...
  describe('DoublePoolWTokens', () => {
    ...
    beforeAll(async () => {
      ...
      poolClients.push(stakingPoolFactory.getAppClientById({ appId: poolKeys[0].poolAppId, defaultSender: validatorOwnerAccount.addr }))
      ...
      poolClients.push(stakingPoolFactory.getAppClientById({ appId: poolKeys[1].poolAppId, defaultSender: validatorOwnerAccount.addr }))
    })
    ...
    test('addStakers', async () => {
      for (let i = 0; i < 2; i += 1) {
        ...
        const stakeAmount = AlgoAmount.MicroAlgos(AlgoAmount.Algos(5000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
        const [stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount, 0n)
      ...
      expect((await getValidatorState(validatorMasterClient, validatorId)).totalAlgoStaked).toEqual(AlgoAmount.Algos(10000).microAlgos)
    })
    ...
    test('extractRewards', async () => {
      for (let i = 0; i < 2; i += 1) {
        ...
        const origStakerAssetBalance = await fixture.context.algorand.asset.getAccountInformation(stakerAccounts[i].addr, rewardTokenId)
        ...
        expect(newStakerBalance.balance.microAlgo).toEqual(origStakerBalance.balance.microAlgo + stakerInfo.balance - removeFees)
        // verify that pending reward token payout came to us
        const newStakerAssetBalance = await fixture.context.algorand.asset.getAccountInformation(stakerAccounts[i].addr, rewardTokenId)
        expect(newStakerAssetBalance.balance).toEqual(origStakerAssetBalance.balance + stakerInfo.rewardTokenBalance)
        ...
        expect(newValidatorState.rewardTokenHeldBack).toEqual(origValidatorState.rewardTokenHeldBack - stakerInfo.rewardTokenBalance)
      }
    })
  })
  ...
  describe('TokenGatingByCreator', () => {
    ...
    describe('stakeTest', () => {
      ...
      test('stakeNoTokenOffered', async () => {
        const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
        await expect(addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)).rejects.toThrowError()
      })

      test('stakeWrongTokenOffered', async () => {
        const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
        await expect(
          addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, stakerCreatedTokenId),
        ).rejects.toThrowError()
      })
  ...
  describe('TokenGatingByAsset', () => {
    ...
    beforeAll(async () => {
      ...
      gatingToken1Id = await createAsset(fixture.context, tokenCreatorAccount, 'Gating Token 1 [Other by same]', 'GATETK1', 10, 0)
      gatingToken2Id = await createAsset(fixture.context, tokenCreatorAccount, 'Gating Token 2 [Required]', 'GATETK2', 10, 0)
    ...
    describe('stakeTest', () => {
      ...
      test('stakeWGatingToken1ShouldFail', async () => {
        const stakeAmount1 = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
        await expect(
          addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, gatingToken1Id),
        ).rejects.toThrowError()
      })
  ...
  describe('TokenGatingMultAssets', () => {
    ...
    beforeAll(async () => {
      ...
      for (let i = 0; i < 4; i += 1) {
        gatingTokens.push(await createAsset(fixture.context, tokenCreatorAccount, `Gating Token ${i}`, `GATETK${i}`, 10, 0))
      }
    ...
    describe('stakeTest', () => {
      ...
      test('stakeWGatingTokens', async () => {
        ...
        for (let i = 1; i < 4; i += 1) {
          await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, AlgoAmount.Algos(1000), gatingTokens[i])
        }
        poolInfo = await getPoolInfo(validatorMasterClient, firstPoolKey)
        expect(poolInfo.totalStakers).toEqual(1n)
  ...
  describe('SaturatedValidator', () => {
    ...
    test('stakeFillingPool', async () => {
      ...
      await expect(
        addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, AlgoAmount.MicroAlgos(1000), 0n),
      ).rejects.toThrowError()
    })
    ...
    test('addPools', async () => {
      ...
      expect(newSoftMax).toEqual(BigInt(Math.min(Number(constraints.maxAlgoPerValidator / 3n), Number(constraints.maxAlgoPerPool))))
    })
    ...
    test('fillNewPools', async () => {
      ...
      ;[poolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, AlgoAmount.MicroAlgos(newSoftMax), 0n)
    ...
    test('testPenalties', async () => {
      ...
      const tmpPoolClient = stakingPoolFactory.getAppClientById({ appId: pools[2].poolAppId, defaultSender: validatorOwnerAccount.addr })
      ...
      if (diminishedRewards > rewardAmount - normalValidatorCommission) {
        consoleLogger.info(`reducing awards from ${diminishedRewards} to ${rewardAmount - normalValidatorCommission}`)
      ...
      expect(newFeeSinkBal.balance.microAlgo).toBeGreaterThanOrEqual(origFeeSinkBal.balance.microAlgo + (rewardAmount - diminishedRewards))
  ...
  describe('StakeAddRemoveBugVerify', () => {
    ...
    test('addRemoveStakers', async () => {
      ...
      const stakeAmt = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
      ...
      ;[poolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakers[1], AlgoAmount.Algos(500), 0n)
      expect(poolKey.id).toEqual(firstPoolKey.id)
  ...
  describe('StakerMultiPoolAddRemoveBugVerify', () => {
    ...
    beforeAll(async () => {
      ...
      validatorIds.push(await addValidator(fixture.context, validatorMasterClient, validatorOwnerAccount, config, mbrs.addValidatorMbr))
      poolKeys.push(
        await addStakingPool(
          fixture.context,
          validatorMasterClient,
    ...
    test('stakeUnstakeReproduce', async () => {
      ...
      const stakeAmt = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)
      let [poolKey] = await addStake(fixture.context, validatorMasterClient, validatorIds[0], stakerAccount, stakeAmt, 0n)
      expect(poolKey.id).toEqual(poolKeys[0].id)
      ;[poolKey] = await addStake(fixture.context, validatorMasterClient, validatorIds[1], stakerAccount, stakeAmt, 0n)
      ...
      const val1Pool = stakingPoolFactory.getAppClientById({ appId: poolKeys[0].poolAppId, defaultSender: stakerAccount.addr })
      ...
      async () => {
        for (let i = 0; i < NumStakers + 1; i += 1) {
          ...
          if (i < NumStakers) {
            ...
            ;[stakedPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount1, 0n)
          } else {
          ...
          expect(poolInfo.totalAlgoStaked).toEqual(stakeAmount1.microAlgos - mbrs.addStakerMbr * BigInt(i + 1))

          expect((await getValidatorState(validatorMasterClient, validatorId)).totalStakers).toEqual(BigInt(i + 1))
        }
      },
      4 * 60 * 1000, // 4 mins
    )
  ...
  describe('CoinFabrik Audit suggested extra tests', () => {
    describe('HI-01 Token Reward Calculation Inconsistent for Partial Stakers', () => {
      ...
      test('Token partial epoch rewards distributed should not affect subsequent distributions during the same epoch update', async () => {
        ...
        const stakeAmount = AlgoAmount.MicroAlgos(AlgoAmount.Algos(1000).microAlgos + AlgoAmount.MicroAlgos(mbrs.addStakerMbr).microAlgos)

        // Add stake for first staker - partial epoch
        const [aPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, partialEpochStaker, stakeAmount, 0n)

        // Add stake for partial-epoch staker
        const [newPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, partialEpochStaker2, stakeAmount, 0n)
        ...
        await verifyRewardAmounts(fixture.context, 0n, tokenRewardPerPayout, stakersPriorToReward, stakersAfterReward, epochRoundLength)
      })
    })
    ...
    describe('ME-02 Incorrect Validator SunsettingOn Verification', () => {
      ...
      test('Cannot stake after sunsetting', async () => {
        ...
        await expect(addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, stakeAmount, 0n)).rejects.toThrowError()
      })
    })
    ...
    describe('ME-03 Incentivizing Pool Saturation for Staker Gain', () => {
      ...
      beforeAll(async () => {
        ...
        pool0Client = stakingPoolFactory.getAppClientById({ appId: pools[0].poolAppId, defaultSender: validatorOwnerAccount.addr })

        pool1Client = stakingPoolFactory.getAppClientById({ appId: pools[0].poolAppId, defaultSender: validatorOwnerAccount.addr })
      ...
      test('Saturation should not benefit stakers', async () => {
        ...
        const validatorTotalStakeAfter = (await getValidatorState(validatorMasterClient, validatorId)).totalAlgoStaked

        // add 2 algo beyond to go into saturation
        const amountToSaturation = AlgoAmount.MicroAlgos(constraints.amtConsideredSaturated - validatorTotalStakeAfter + 1n)

        const [aPoolKey] = await addStake(fixture.context, validatorMasterClient, validatorId, stakerAccount, amountToSaturation, 0n)

        expect(aPoolKey.poolId).toEqual(3n)

        const validatorTotalStakeAfterSaturation = (await getValidatorState(validatorMasterClient, validatorId)).totalAlgoStaked

        expect(validatorTotalStakeAfterSaturation).toEqual(validatorTotalStakeAfter + amountToSaturation.microAlgos)
